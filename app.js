(() => {
  ...
  const state={
    ...
    contactMode:false,selectedPhotoIds:new Set(),bulkSelectMode:false,bulkSelectedIds:new Set(),contactBlob:null,photoModalId:null,photoModalDirty:false,rollModalId:null,presetAutoDate:false,presetAutoFrame:false,captureMode:'photo',mediaRecorder:null,videoChunks:[],recording:false,recordStartedAt:0,recordTimer:null,videoAudioStream:null,pendingShareFile:null,pendingShareTitle:'',photosQueueIds:[],photoProcessQueue:[],photoProcessing:false,captureSequence:0,cameraTorchOn:false,cameraCapabilities:null,cameraZoomTimer:null,cameraFlashMode:localStorage.getItem('kira.cameraFlashMode')==='on'?'on':'off',pendingSystemCapture:null,systemCaptureFocusTimer:null,bestQualitySessionActive:false,bestQualitySessionShots:0,developInitialized:false,cameraImmersive:false
  };
  ...
  const FILTER_IDENTITY_GAIN=Object.freeze({
  ...
  const beautyBuffers={still:{},live:{}};
  ...
  function liveParamsForPreset(f){if(f.kind==='recipe'&&f.snapshot){return filterParamsForSnapshot(f.snapshot)}const mix=(state.filterIntensity/100)*presetIdentityGain(f),p=f.p||{};return {exposure:Number(p.exposure||0)*mix,brightness:Number(p.brightness||0)*mix,contrast:Number(p.contrast||0)*mix,saturation:Number(p.saturation||0)*mix,warmth:Number(p.warmth||0)*mix,tint:Number(p.tint||0)*mix,fade:Number(p.fade||0)*mix,vignette:Number(p.vignette||0)*mix,bloom:Number(p.bloom||0)*mix,sepia:Number(p.sepia||0)*mix,hue:Number(p.hue||0)*mix,castColor:p.castColor||null,castStrength:Number(p.castStrength||0)*mix,castMode:p.castMode||'soft-light',softness:Number(p.softness||0)*mix,lowRes:Number(p.lowRes||0)*mix,scanlines:Number(p.scanlines||0)*mix,mono:monoLookActive(f,state.filterIntensity)}}
  function currentLiveParams(){const p=filterParams();return {exposure:p.exposure,brightness:p.brightness,contrast:p.contrast,saturation:p.saturation,warmth:p.warmth,tint:p.tint,fade:p.fade,vignette:p.vignette,bloom:p.bloom,sepia:p.sepia,hue:p.hue,castColor:p.castColor,castStrength:p.castStrength,castMode:p.castMode,softness:p.softness,lowRes:p.lowRes,scanlines:p.scanlines,mono:p.mono}}
  ...
  function setCaptureMode(mode){
    ...
    if(state.captureMode==='video'&&state.bestQualitySessionActive)finishBestQualitySession(true);
  ...
  function currentPhotoCaptureSource(){return state.settings.photoCaptureSource==='live'?'live':'system'}
  ...
  function applyPreviewToneToCanvas(ctx,w,h,p){
  ...
  function photoQualityProfile(mode=state.settings.photoQuality||'high'){
  ...
  function exportDimensions(){const iw=state.image.naturalWidth,ih=state.image.naturalHeight,max=state.exportQuality==='Original'?4096:state.exportQuality==='High'?3072:1440,sc=Math.min(1,max/Math.max(iw,ih));return [Math.max(1,Math.round(iw*sc)),Math.max(1,Math.round(ih*sc))]}
  function currentBlob(type='image/jpeg',quality=.94){return new Promise(resolve=>{const [w,h]=exportDimensions(),c=document.createElement('canvas');c.width=w;c.height=h;drawEdited(c,filterParams(),true);const q=state.exportQuality==='Social'?.9:state.exportQuality==='Original'?.97:Math.max(.96,quality);c.toBlob(resolve,type,q)})}
  ...
  function closeModal(id){const el=$('#'+id);if(el)el.classList.add('hidden');if(id==='photoModal'){state.photoModalDirty=false;for(const media of [$('#photoModalImage'),$('#photoModalVideo')]){const u=media?.dataset.objectUrl;if(u){URL.revokeObjectURL(u);delete media.dataset.objectUrl}if(media?.tagName==='VIDEO'){media.pause();media.removeAttribute('src');media.load()}}}}
  ...
  function modalBrowseItems(){return currentRollItems().filter(x=>x?.blob)}
  ...
  function bindInputs(){bindRollGridInteractions();bindCameraBeautyControls();
    ...
    $('#galleryBtn').onclick=()=>$('#galleryInput').click();$('#shutterBtn').onclick=captureOrRecord;$$('[data-capture-mode]').forEach(b=>b.onclick=()=>setCaptureMode(b.dataset.captureMode));$('#cameraSourceBtn')&&($('#cameraSourceBtn').onclick=togglePhotoCaptureSource);$('#bestQualityShootAgainBtn')&&($('#bestQualityShootAgainBtn').onclick=beginSystemCameraCapture);$('#bestQualityDoneBtn')&&($('#bestQualityDoneBtn').onclick=()=>finishBestQualitySession());$('#flipCameraBtn').onclick=flipCamera;$('#startCameraBtn').onclick=()=>{if(!navigator.mediaDevices?.getUserMedia){beginSystemCameraCapture();return}startCamera(true)};
  ...
  async function setupServiceWorkerUpdates(){
    ...
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=1.0.10');
