(() => {
  ...
  function cameraPresetCard(f){const fav=f.kind==='recipe'?(f.pinned?'♥':''):(state.favoriteFilters.has(f.name)?'♥':'');return `<button class="preset-card ${cameraPresetIsActive(f)?'active':''}" data-camera-filter="${escapeHtml(f.name)}" data-kind="${f.kind}" ${f.recipeId?`data-recipe-id="${f.recipeId}"`:''}><canvas class="preset-thumb live-thumb" width="164" height="196" data-live-filter="${escapeHtml(f.name)}" data-live-kind="${f.kind}" ${f.recipeId?`data-live-recipe="${f.recipeId}"`:''}></canvas><span>${escapeHtml(f.name)}${fav?` <i class="favorite-star">${fav}</i>`:''}</span><small style="font-size:9px;color:#8f7072">${f.kind==='recipe'?'Recipe':(f.p?.pack||f.cat)}</small></button>`}
  ...
  function renderCameraCategories(){const cats=['Kira','Camera Packs','Instant','Vintage','Date Cam','Film','Film Stock','CCD','Y2K','Dream','Japan','Favorites','My Recipes','All'];const area=$('#cameraCategories');if(!area)return;area.innerHTML=cats.map(c=>`<button class="chip ${state.activeCameraCategory===c?'active':''}" data-camera-cat="${c}">${c}</button>`).join('');area.onclick=e=>{const b=e.target.closest('[data-camera-cat]');if(!b)return;state.activeCameraCategory=b.dataset.cameraCat;renderCameraCategories();renderCameraFilters();haptic()}}
  function renderCameraFilters(){const area=$('#cameraStrip');if(!area)return;const list=visibleCameraPresets();area.innerHTML=list.length?list.map(cameraPresetCard).join(''):'<div class="notice">No filters saved in this category yet.</div>';$$('[data-camera-filter]',area).forEach(btn=>btn.onclick=()=>{if(state.recording){toast('Finish recording before changing looks.');return}if(btn.dataset.kind==='recipe'&&btn.dataset.recipeId){fastCameraApplyRecipe(btn.dataset.recipeId)}else{fastCameraSelect(btn.dataset.cameraFilter)}syncCameraActiveCards();applyLiveFilter();requestLiveThumbPaint(true)});requestLiveThumbPaint(true)}
  ...
  function fastCameraSelect(name){state.activeFilter=name;state.selectedRecipeId=null;applyPresetExtras(name);if(state.settings.rememberFilter)localStorage.setItem('kira.lastFilter',name);updateCameraHUD();saveSettings();haptic()}
  function fastCameraApplyRecipe(id){const recipe=state.recipes.find(x=>x.id===id);if(!recipe)return;const snap=JSON.parse(JSON.stringify(recipe.snapshot||{}));Object.assign(state,snap);state.selectedRecipeId=id;state.presetAutoDate=false;state.presetAutoFrame=false;updateCameraHUD();saveSettings();haptic()}
  ...
  function bindInputs(){
    ...
    $('#cameraStrip')?.addEventListener('scroll',()=>requestLiveThumbPaint(false),{passive:true});
    ...
    const fi=$('#filterIntensity');rangeHistory(fi);fi.oninput=e=>{state.filterIntensity=Number(e.target.value);$('#intensityValue').textContent=e.target.value;$('#activeLookIntensity')&&($('#activeLookIntensity').textContent=`${e.target.value}%`);$('#liveFilterIntensity').value=e.target.value;scheduleRender();applyLiveFilter()};fi.onchange=()=>{finishRangeHistory();applyLiveFilter();requestLiveThumbPaint(true)};
    const live=$('#liveFilterIntensity');live.oninput=e=>{state.filterIntensity=Number(e.target.value);$('#filterIntensity').value=e.target.value;$('#intensityValue').textContent=e.target.value;$('#liveIntensityValue').textContent=e.target.value;$('#activeLookIntensity')&&($('#activeLookIntensity').textContent=`${e.target.value}%`);applyLiveFilter()};live.onchange=()=>requestLiveThumbPaint(true);
