(() => {
  ...
  const state={
    ...
    adjustments:defaultAdjust(),effects:defaultEffects(),frame:'None',frameTone:'#fff8f1',frameWidth:8,frameCorner:8,caption:'',captionFont:'Classic Serif',
  ...
  function captionFontCss(name,size){const map={'Classic Serif':`italic ${size}px Georgia, 'Times New Roman', serif`,'1989 Sparkle':`italic ${size}px 'Snell Roundhand', 'SignPainter', 'Segoe Script', 'Brush Script MT', 'Apple Chancery', cursive`,'Typewriter':`${size}px 'Courier New', 'American Typewriter', ui-monospace, monospace`,'Marker':`italic ${size}px 'Bradley Hand', 'Segoe Print', 'Comic Sans MS', cursive`,'Mono Label':`${size}px ui-monospace, SFMono-Regular, Menlo, Monaco, monospace`};return map[name]||map['Classic Serif']}
  ...
  function drawFrame(ctx,w,h){
    ...
    if(state.frame==='Classic'||state.frame==='Mini Print'||state.frame==='Postcard'||instant.includes(state.frame)){
      ...
      if(instant.includes(state.frame)||state.frame==='Mini Print'||state.frame==='Postcard'){
        ...
        if(state.caption){ctx.fillStyle=black?'#f8eee7':state.frame==='Postcard'?'#8b5d4b':'#6a4d4e';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=captionFontCss(state.captionFont,Math.max(15,Math.round(w*.031)));ctx.fillText(state.caption,w/2,h-footer*.36)}
      }
  ...
  function drawCameraShotFast(canvas,source){const ctx=canvas.getContext('2d',{alpha:false}),w=canvas.width,h=canvas.height,p=filterParams();ctx.save();ctx.fillStyle='#171414';ctx.fillRect(0,0,w,h);ctx.filter=cameraCssFromParams(p);ctx.drawImage(source,0,0,w,h);ctx.filter='none';if(p.fade>0){ctx.globalAlpha=Math.min(.30,p.fade/110);ctx.fillStyle='#ead9c9';ctx.fillRect(0,0,w,h);ctx.globalAlpha=1}if(p.warmth){ctx.globalCompositeOperation='soft-light';ctx.globalAlpha=Math.min(.22,Math.abs(p.warmth)/150);ctx.fillStyle=p.warmth>0?'#ff995e':'#4d94c2';ctx.fillRect(0,0,w,h);ctx.globalCompositeOperation='source-over';ctx.globalAlpha=1}if(p.bloom>0&&p.bloom<30){ctx.globalCompositeOperation='screen';ctx.globalAlpha=Math.min(.12,p.bloom/220);ctx.filter='blur(3px) brightness(112%)';ctx.drawImage(source,0,0,w,h);ctx.filter='none';ctx.globalCompositeOperation='source-over';ctx.globalAlpha=1}if(p.vignette>0){const g=ctx.createRadialGradient(w/2,h/2,Math.min(w,h)*.22,w/2,h/2,Math.max(w,h)*.72);g.addColorStop(.5,'rgba(0,0,0,0)');g.addColorStop(1,`rgba(20,8,8,${Math.min(.42,p.vignette/105)})`);ctx.fillStyle=g;ctx.fillRect(0,0,w,h)}if(state.frame!=='None')drawFrame(ctx,w,h);if(state.dateEnabled)drawDate(ctx,w,h);ctx.restore()}
  async function processContinuousPhoto(task){const source=await decodePhotoBlob(task.blob);try{const maxSide=1920,sw=source.width||source.naturalWidth||task.width,sh=source.height||source.naturalHeight||task.height,scale=Math.min(1,maxSide/Math.max(sw,sh)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(sw*scale));c.height=Math.max(1,Math.round(sh*scale));drawCameraShotFast(c,source);const finalBlob=await new Promise(resolve=>c.toBlob(resolve,'image/jpeg',.92));if(!finalBlob)throw new Error('Could not encode photo');const name=`kira-${task.stamp}`;const id=await storeRollPhoto(finalBlob,{kind:'edited',mediaType:'photo',name,filter:task.filter,favorite:false,snapshot:task.snapshot,rollId:task.rollId,cameraCapture:true});queueRollIdForPhotos(id);toast(state.settings.autoPhotos?`Saved • ${state.photosQueueIds.length} waiting for Photos`:`Saved to ${rollName(task.rollId)} ✓`)}finally{if(source&&typeof source.close==='function')source.close()}}
