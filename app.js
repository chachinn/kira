(() => {
  ...
  const defaultSettings={grid:false,haptics:true,rememberFilter:true,keepOriginal:true,autoSave:true,autoPhotos:true,continuousShoot:true,videoAudio:true,videoQuality:'smooth'};
  ...
  const state={
    ...
    contactMode:false,selectedPhotoIds:new Set(),bulkSelectMode:false,bulkSelectedIds:new Set(),contactBlob:null,photoModalId:null,rollModalId:null,presetAutoDate:false,presetAutoFrame:false,captureMode:'photo',mediaRecorder:null,videoChunks:[],recording:false,recordStartedAt:0,recordTimer:null,videoAudioStream:null,pendingShareFile:null,pendingShareTitle:'',photosQueueIds:[],photoProcessQueue:[],photoProcessing:false,captureSequence:0
  };
  ...
  const settingsVersion=Number(localStorage.getItem('kira.settingsVersion')||0);if(settingsVersion<10){state.settings.continuousShoot=true;state.settings.autoPhotos=true;localStorage.setItem('kira.settingsVersion','10');localStorage.setItem('kira.settings',JSON.stringify(state.settings));}
